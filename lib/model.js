...
class Model {
  static init (schema, options = {}) {
    ...
    if (this.OPTIONS.requireAuth && !has(this.CONFIG, 'getAuthToken')) {
    ...
    // Use a symbol for collection
    this.collectionId = Symbol.for('collection')
  ...
  *****************************************************************/
  ...
  static _getDefaultValue (fieldname, primaryKey = null) {
    const fieldconf = this.schema[fieldname]

    const defaultValue = isFunction(fieldconf.defaultValue)
  ...
  static _buildRawItem (item = {}) {
    ...
    const primaryKey = item[this.primaryKey] || item['primaryKey'] || this._getDefaultValue(this.primaryKey)
    ...
    for (const [fieldname, fieldconf] of this.schemaArray) {
      if (fieldname === this.primaryKey) {
        ...
        rawItem[fieldname] = this._getDefaultValue(fieldname, primaryKey)
      }
    }
  ...
  constructor (data, options = {}) {
    ...
    if (isObject(data)) {
      ...
      // Automatically add the primaryKey if missing on a new instance
      if (isNew && !has(this, this.constructor.primaryKey)) {
        this[this.constructor.primaryKey] = this.constructor._getDefaultValue(this.constructor.primaryKey)
      }
  ...
  *****************************************************************/
   ...
   */
  isCollection () {
    return has(this, this.constructor.collectionId)
  }
  ...
  setCollection (newCollection) {
    this[this.constructor.collectionId] = newCollection
  }
   ...
   */
  items () {
    return this[this.constructor.collectionId]
  }
   ...
   */
  find (primaryKey) {
    return this.items().find(item => item[this.constructor.primaryKey] === primaryKey) || null
  }
