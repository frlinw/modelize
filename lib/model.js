...
class Model {
  static init (schema, options = {}) {
    ...
    if (this.requireAuth && !has(this.CONFIG, 'authToken')) {
      throw new Error(`[Modelize][Config] 'authToken' is required if 'requireAuth' is set to true. try Modelize.config({ authToken: () => localStorage.get('authToken')) })`, schema)
    }
    ...
    for (const [fieldname, fieldconf] of this.schemaEntries) {
      ...
      if (!has(fieldconf, 'type')) {
        throw new Error(`[Modelize][Model] Required property 'type' is missing from field '${fieldname}'`, schema)
      }
  ...
  *****************************************************************/
  ...
  static async _buildRequestInit (data, method, signal = null) {
    ...
    if (this.requireAuth) {
      ...
      if (!token) {
        throw new Error(`[Modelize][Fetch] Impossible to get the auth token to access ${this.endpoint}`)
      }
  ...
  *****************************************************************/
  ...
  _beforeSaveItem () {
    ...
    for (const [fieldname, validator] of Object.entries(this.$modelize.validator)) {
  ...
  async fetch (options) {
    if (!this.constructor.endpoint) {
      throw new Error('[Modelize][Fetch] An endpoint is required to perform a request')
    }
