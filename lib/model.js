...
class Model {
  static init (schema, options = {}) {
    ...
    // Clone the schema
    this.schema = Object.assign({}, schema) // waiting for edge support https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Spread_syntax
    ...
    if (isNull(this.primaryKey)) {
      throw new Error(`[Modelize.js] Initialization error: 'primaryKey' not found in the schema`, schema)
    }
  ...
  *****************************************************************/
  ...
  constructor (data, options = {}) {
    const isFromDb = options.isFromDb || false
    ...
    if (isObject(data)) {
      ...
      for (const [fieldname, fieldconf] of this.constructor.schemaArray) {
        this.$todb[fieldname] = {
          toSend: fieldconf.primaryKey || ['createdAt', 'updatedAt'].includes(fieldname) || false,
          isValid: (value, data) => {
  ...
  async fetch (options) {
    ...
    const formattedData = new this.constructor(data, { isFromDb: true })
