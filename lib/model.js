...
class Model {
  static init (schema, options = {}) {
    if (!this.CONFIG.baseUrl) {
      throw new Error(`[Modelize.js] Config error: baseUrl must be defined. try Modelize.config({ baseUrl: 'https://api.example.com' })`, schema)
    }
  ...
  *****************************************************************/
  ...
  static _buildRequestInit (data, method, signal = null) {
    ...
    if (['POST', 'PUT', 'PATCH'].includes(method)) {
      ...
      init.body = JSON.stringify(this._beforeSave(data))
    }
    ...
    if (isObject(data)) {
      ...
      for (const [fieldname, value] of Object.entries(data)) {
        ...
        if (
          ...
          newValue = fieldconf.type.afterFetch(value)
        }
    ...
    for (const fielditem of fieldlist) {
      ...
      // Recursive validation for nested collections
        ...
        if (has(this, fieldname)) {
          ...
          // Check if each item of the collection are valid
          for (const item of this[fieldname].list) {
            const childResult = item._validQuietly(fieldlist)
