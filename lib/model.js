...
export default class Model {
  static config (options = {}) {
    ...
    if (this.CONFIG.requireAuth && !has(options, 'getAuthToken')) {
    ...
    if (has(options, 'getAuthToken') && isFunction(options.getAuthToken)) {
      this.CONFIG.getAuthToken = options.getAuthToken
    }
  ...
  static init (schema, options = {}) {
    if (!this.CONFIG.baseUrl) {
      throw new Error(`[Modelize.js] Config error: baseUrl must be defined: Model.config({ baseUrl: 'https://api.example.com' })`, schema)
    }
    ...
    if (this.INIT.requireAuth && !('getAuthToken' in this.CONFIG)) {
      throw new Error(`[Modelize.js] Config error: 'getAuthToken' is required if 'requireAuth' is set to true. Model.config({ getAuthToken: () => localStorage.get('authToken')) })`, schema)
    }
  ...
  *****************************************************************/
  ...
  static _stringify (data) {
    ...
    if (isObject(data)) {
      ...
      if (has(data, '$count')) {
        return this._stringifyCollection(data)
      } else {
        return this._stringifyItem(data)
      }
    }
  }

  /** Format data recursively based on schema definition */
  static _stringifyCollection (collection) {
    return collection.list.map(item => this._stringifyItem(item))
  }
  ...
  static _stringifyItem (item) {
    ...
    for (const [fieldname, dbcheck] of Object.entries(item.$todb)) {
      ...
      if (dbcheck.toSend && dbcheck.isValid(value, item)) {
        ...
        newItem[fieldname] = fieldconf.type.stringify(value)
      }
    }
  ...
  *****************************************************************/
    ...
    if (has(options, 'params')) {
      url += `?${
        Object.entries(options.params)
          .map(([key, value]) => {
            if (this.CONFIG.isDate(value)) {
              value = this.CONFIG.toDate(value).toISOString()
            }
  ...
  static _buildRequestInit (data, method, signal = null) {
    ...
    if (this.INIT.requireAuth) {
      const token = this.CONFIG.getAuthToken()

      if (!token) {
        throw new Error(`[Modelize.js] Fetch Error: anonymous user cannot access '${this.INIT.endpoint}' private API`)
      }
    ...
    if (['POST', 'PUT', 'PATCH'].includes(method)) {
      ...
      init.body = JSON.stringify(this._stringify(data))
    }
  ...
  *****************************************************************/
  ...
  static buildRaw (item = {}) {
    ...
    // Generate default pk
    let primaryKey = item[this.primaryKey] || item['primaryKey']
  ...
  constructor (data, isFromDb = false) {
    ...
    if (isObject(data)) {
      ...
      for (const [fieldname, fieldconf] of Object.entries(this.constructor.schema)) {
        this.$todb[fieldname] = {
          toSend: fieldconf.primaryKey || fieldconf.alwaysSend || false,
          isValid: (value, data) => {
      ...
      for (const [fieldname, value] of Object.entries(data)) {
        ...
        if (
          ...
          newValue = fieldconf.type.parseDate(value)
        }
   ...
   */
  async fetch (options) {
    if (!this.constructor.INIT.endpoint) {
