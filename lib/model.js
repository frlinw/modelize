...
class Model {
  static config (options = {}) {
    ...
    if (this.requireAuth && !this.authToken) {
      throw new Error(`[Modelize][Config] authToken is required if requireAuth is activated`)
    }

    if (
      options.collectionPattern &&
      has(options.collectionPattern, 'count') &&
      has(options.collectionPattern, 'data')
    ) {
  ...
  static init (schema, options = {}) {
    ...
    if (this.requireAuth && !this.authToken) {
      throw new Error(`[Modelize][Init] authToken is required if requireAuth is activated`)
    }
  ...
  *****************************************************************/
  ...
  async fetch (options) {
    ...
    const matchCollectionPattern = this.constructor.matchCollectionPattern(serverData)

    // Save original data from server (for comparison before save)
    const data = matchCollectionPattern
      ? serverData[this.constructor.collectionPattern.data]
      : serverData
