...
class Model {
  static init (schema, options = {}) {
    ...
    for (const [fieldname, fieldconf] of this.schemaArray) {
      ...
      // Set default value & unsure it is a function (avoid type check on runtime)
      if (!has(fieldconf, 'defaultValue')) {
        fieldconf.defaultValue = fieldconf.type.defaultValue
      }

      if (!isFunction(fieldconf, 'defaultValue')) {
        fieldconf.defaultValue = () => fieldconf.defaultValue
      }
  ...
  *****************************************************************/
  ...
  constructor (data, options = {}) {
    ...
    if (isObject(data)) {
      ...
      if (isNew && !has(this, this.constructor.primaryKeyFieldname)) {
        this[this.constructor.primaryKeyFieldname] = this.constructor.schema[this.constructor.primaryKeyFieldname].defaultValue()
      }
