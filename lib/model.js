...
export default class Model {
  static config (options = {}) {
    ...
    this.requireAuth = has(options, 'requireAuth')
      ? options.requireAuth
      : false

    if (this.requireAuth && !has(options, 'getAuthToken')) {
    ...
    this.isDate = has(options, 'isDate') && isFunction(options.isDate)
      ? options.isDate
      : (value) => value instanceof Date && !isNaN(value.valueOf())

    this.parseDate = has(options, 'parseDate') && isFunction(options.parseDate)
      ? options.parseDate
      : (dateString) => new Date(dateString)

    this.toDate = has(options, 'toDate') && isFunction(options.toDate)
      ? options.toDate
      : (value) => value
  ...
  static init (schema, config = {}) {
    ...
    if (this.config.requireAuth && !has(this, 'getAuthToken')) {
