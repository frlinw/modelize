...
export default class Model {
  static init (schema, config = {}) {
    ...
    this.BASE_API_URL = this.config.endpoint
      ? `${ENV.BACKEND_URL}/${this.config.endpoint}`
      : ENV.BACKEND_URL
    ...
    for (const [fieldname, fieldconf] of Object.entries(this.schema)) {
      ...
      if (!has(fieldconf, 'default')) {
        fieldconf.default = fieldconf.type.default
      }
      ...
      url += `?${
        Object.entries(options.params)
          .map(([key, value]) => {
            ...
            if (isDate(value)) {
              value = formatDate(value, DATETIMEFORMAT)
            }
  ...
  *****************************************************************/
  ...
  static buildRaw (item = {}) {
    ...
    if (!pk) {
      ...
      pk = isFunction(fieldconf.default)
        ? fieldconf.default()
        : fieldconf.default
    }
    ...
    for (const [fieldname, fieldconf] of Object.entries(this.schema)) {
      ...
      if (value === undefined) {
        if (fieldname === this.primaryKey) {
          ...
          value = isFunction(fieldconf.default)
            ? fieldconf.default(pk)
            : fieldconf.default
        }
      }
   ...
   */
  async fetch (options) {
    if (!this.constructor.config.endpoint) {
